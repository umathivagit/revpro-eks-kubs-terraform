...
resource "aws_iam_policy" "eks-tagging" {
  name        = "prod_resource_tagging_for_eks"
  path        = "/"
  description = "prod_resource_tagging_for_eks"
...
resource "aws_security_group" "worker-node-sg" {
  ...
  description = "Security group for all nodes in the prod cluster"
  vpc_id      = "${var.vpc_id}"
